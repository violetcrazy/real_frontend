...
{% block title %}Search Result - {{ ' Page ' ~ page }} {% endblock %}
{% block description %}Search Result - {{ ' Page ' ~ page }} {% endblock %}
{% block keywords %}Search Result - {{ ' Page ' ~ page }} {% endblock %}
        ...
        <div class="row">
            <div class="col-xs-3">
                <form id="form-search" action="" method="GET" enctype="multipart/form-data" class="form-search">
                    ...
                    {% include 'default_eng/search/_filter.volt' %}
                </form>
            </div>
            <div class="col-xs-9">
                <h2 class="title-block title-page-search">Search Results</h2>
                <div id="mess-save-search"></div>
                <div class="des-title">We found <b class="text-danger"> {{ total_items }} </b> properties with availability
                    {% if (total_items > 0) %}
                        Showing
                        <b class="text-danger">
                ...
                <div class="sort-wrap">
                    <span class="text-intro">Sort by</span>
                        ...
                        <div class="item">
                            <div class="dropdown custom-select">
                                ...
                                <button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="btn btn-default btn-block dropdown-toggle">
                                    <span class="text">Show {% if limit is defined %}{{ limit }}{% endif %}</span>
                ...
                success: function (response) {
                    if (typeof response != "undefined") {
                        if (response.status == 200) {
                            var html = '<div class="alert alert-success">Save success!</div>';
                            ...
                            if ( $('#link-login').size() > 0 ) {
                                ...
                                var html = '<div class="alert alert-error">Error. Please try again.</div>';
                                $('#mess-save-search').html(html)
                            }
                        }
                    } else {
